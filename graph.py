...
async def parse_input(state: AgentTaskState) -> Dict[str, Any]:
    ...
    decomposed_steps = _decompose_task_into_steps(
...
def _decompose_task_into_steps(
    ...
    # Try running in existing event loop, else run synchronously
    steps = None
    try:
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                future = pool.submit(asyncio.run, call_ollama())
                raw_steps = future.result(timeout=20)
        else:
            raw_steps = asyncio.run(call_ollama())
...
async def route_task(state: AgentTaskState) -> Dict[str, Any]:
    ...
    if state.task_steps and state.active_step_index < len(state.task_steps):
        ...
        tool_arguments = _generate_tool_arguments(
...
def _generate_tool_arguments(
    ...
    if prompt:
        try:
            import concurrent.futures
            with concurrent.futures.ThreadPoolExecutor() as pool:
                future = pool.submit(asyncio.run, call_ollama_extract(prompt))
                extracted = future.result(timeout=15)
            logger.info(
...
async def execute_tool(state: AgentTaskState) -> Dict[str, Any]:
    ...
    if updated_steps and state.active_step_index < len(updated_steps):
        updated_steps[state.active_step_index]["status"] = "completed"
        updated_steps[state.active_step_index]["execution_result"] = execution_result
...
def build_aegisflow_graph() -> CompiledStateGraph:
    ...
    # After execute_tool completes, terminate the graph for this step.
    builder.add_edge("execute_tool", END)
    logger.debug("Edge added: execute_tool ──► END")
